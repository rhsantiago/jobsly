...
include "serverlogconfig.php";
 $database = new Database();   
                                            ...
                                            if(!empty($search)){                                       
                                                $search='%'.$search.'%';
                                                $database->query('SELECT useraccounts.id,fname, lname, email,signupdate, isverified from personalinformation, useraccounts where personalinformation.userid=useraccounts.id and (fname like :search or lname like :search or email like :search) order by signupdate desc limit '.$next.',10');
                                                $database->bind(':search', $search);                                                
                                            }else{
                                                $database->query('SELECT useraccounts.id,fname, lname, email,signupdate, isverified from personalinformation, useraccounts where personalinformation.userid=useraccounts.id  order by signupdate desc limit '.$next.',10');                                            
                                            }   
